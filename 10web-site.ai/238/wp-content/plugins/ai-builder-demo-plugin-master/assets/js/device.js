jQuery(window).load(function() {
    if (window.self === window.top && window.innerWidth > 860) {
        ...
        setTimeout(function() {
            ...
            jQuery('#tbdemo-device-iframe').load(function() {
                ...
                iframe.find("html, body").addClass("tbdemo-preview-iframe");
                const style = '<style>html { margin-top: 0 !important; }</style>';
                iframe.find("head").append(style);
                iframe.find(".twbb-topbar-domain").remove();
            });
            jQuery(document).on("click", ".tbdemo-device-mobile", function() {
                if (jQuery(this).hasClass("tbdemo-device-active")) {
                    return
                }
                jQuery(document).find(".twbb-topbar-domain-fakeEditor").width((jQuery(window).outerWidth() - 222));
                let window_width = jQuery("body").width();

                let height = "";
                let buttons_container = jQuery('body .tbdemo-devicebar-border:visible');
                let height_px = '100%';
                if (jQuery("body").hasClass("twbb-b")) {
                    if (jQuery(document).find(".twbb-topbar-domain").length) {
                        height = jQuery(window).height() - 130;
                        top_height = 115;
                    } else {
                        height = jQuery(window).height() - 90;
                        top_height = 70;
                    }
                    if (jQuery("body").hasClass("tbdemo-customize-mode")) {
                        jQuery(document).find("#twbb-ab-topbar, .twbb-topbar-domain").innerWidth(window.innerWidth - 212);
                    }
                } else if (buttons_container.length) {
    ...
    jQuery(document).on("click", ".tbdemo-device-laptop", function() {
        ...
        let top = 0;
        let height = '100%';
        if (jQuery("body").hasClass("twbb-b") && jQuery("body").hasClass("tbdemo-customize-mode")) {
            jQuery(document).find("#twbb-ab-topbar").width(window.innerWidth - 270);
            jQuery(document).find(".twbb-topbar-domain").width(window.innerWidth - 230);
        }
        if (typeof tbdemo_fake_editor_marker == 'function' && jQuery(".tbdemo-editor-cont").length) {
        ...
        jQuery(document).find('#tbdemo-device-iframe').animate({
            ...
            jQuery(document).find(".twbb-topbar-domain-fakeEditor").width((jQuery(window).width() - 222));
        });
    });
